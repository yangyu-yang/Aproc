...
typedef struct
{
	uint32_t IndexInFolder[FOLDER_TYPE_MAX];// curremt song play number 
	uint32_t OffsetInFolder[FOLDER_TYPE_MAX][2];// [0]:current song [1]:next song
	uint16_t PlayTime[FOLDER_TYPE_MAX];	
	FolderPlayType FolderType;
} StoryFolderSaveInfo;
