 ...
 *          This legacy version is for backward compatible purpose only.
 *
...
#ifndef __DRC_LEGACY_H__
#define __DRC_LEGACY_H__

#include <stdint.h>


/** error code for DRC legacy */
typedef enum _DRC_LEGACY_ERROR_CODE
{
    DRC_LEGACY_ERROR_SAMPLE_RATE_NOT_SUPPORTED = -256,
    DRC_LEGACY_ERROR_NUM_CHANNELS_NOT_SUPPORTED,
	DRC_LEGACY_ERROR_THRESHOLD_OUT_OF_RANGE,
	DRC_LEGACY_ERROR_RATIO_OUT_OF_RANGE,
	// No Error
	DRC_LEGACY_ERROR_OK = 0,					/**< no error              */
} DRC_LEGACY_ERROR_CODE;


/** DRC legacy mode */
typedef enum _DRC_LEGACY_MODE
{
	DRC_LEGACY_MODE_FULLBAND,					/** full band */
	
	DRC_LEGACY_MODE_CF_B1,						/** 2-band only (Butterworth filters, order = 1) */
	DRC_LEGACY_MODE_CF_LR4,					/** 2-band only (Linkwitz-Riley filters, order = 4) */
	DRC_LEGACY_MODE_CF_Q4,						/** 2-band only (Q controlled filters, order = 4) */
	
	DRC_LEGACY_MODE_CF_B1_FULLBAND,			/** 2-band (Butterworth filters, order = 1) + full band */
	DRC_LEGACY_MODE_CF_LR4_FULLBAND,			/** 2-band (Linkwitz-Riley filters, order = 4) + full band  */
	DRC_LEGACY_MODE_CF_Q4_FULLBAND,			/** 2-band (Q controlled filters, order = 4) + full band */

} DRC_LEGACY_MODE;


/** DRC context */
typedef struct _DRCLegacyContext
{
 ...
 */
int32_t drc_legacy_init(DRCLegacyContext *ct, int32_t num_channels, int32_t sample_rate, 
				 int32_t fc, DRC_LEGACY_MODE mode, uint16_t q[2],
 ...
 * @return error code. DRC_ERROR_OK means successful, other codes indicate error.
 */
int32_t drc_legacy_apply(DRCLegacyContext*ct, int16_t *pcm_in, int16_t *pcm_out, int32_t n, int32_t pregain1, int32_t pregain2);
 ...
 * @return error code. DRC_ERROR_OK means successful, other codes indicate error.
 */
int32_t drc_legacy_apply24(DRCLegacyContext*ct, int32_t *pcm_in, int32_t *pcm_out, int32_t n, int32_t pregain1, int32_t pregain2);
